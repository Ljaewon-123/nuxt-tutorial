<template>
  <v-app>
    <div v-if="errorStatus.statusCode == 600">
      로딩이야요 GOOD!! very well! {{ errorStatus }}
    </div>
    <v-empty-state v-else
      icon="mdi-magnify"
      :headline="String(errorStatus.statusCode)"
      :title="errorStatus.statusMessage"
    ...
    try{
      const verification = await $fetch<{ resultcode: number }>('/api/auth/verification', {
        ...
        async onResponseError({ request, response, options }) {
          ...
          code.value = status
          text.value = response._data.data.message
        }
      })
      ...
      // return abortNavigation({
        ...
        // })
        errorStatus.value.statusCode = code.value
